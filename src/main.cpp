...
// Copyright (c) 2013-2014 Furrycoin Developers
...
uint256 hashGenesisBlock("0x1194f75e847cdcdaacb2a67a1c0c366388edaf3dcf033ee31f94348c232ad857");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Furrycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Furrycoin Signed Message:\n";
...
{
    // Furrycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespanV2 = 300;
static const int64 nTargetSpacingV2 = 300;
...
unsigned int static GetNextWorkRequired_V3(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // thanks to RealSolid & WDC for the basis of this code, modified by Mammix2 for Furrycoin
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Furrycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Furrycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x1194f75e847cdcdaacb2a67a1c0c366388edaf3dcf033ee31f94348c232ad857"); //testnet hashGenesisBlock
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
               ...
               block.nVersion = 1;
               block.nTime    = 1399019284;
               block.nBits    = 0x1e0ffff0;
               block.nNonce   = 2086774377;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Furrycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FurrycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FurrycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FurrycoinMiner(CWallet *pwallet)
{
    printf("FurrycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("furrycoin-miner");
    ...
    try { loop {
        ...
        printf("Running FurrycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FurrycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FurrycoinMiner, pwallet));
}
